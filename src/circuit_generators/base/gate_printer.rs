...
impl ProgramVisitor for ProgramPrinter {
    ...
    fn visit_gate_def(&mut self, name: &Span<Symbol>, params: &[Span<Symbol>], args: &[Span<Symbol>], body: &[Span<Stmt>]) -> Result<(), Self::Error> {
        // ignore definitions
        Ok(())
    }

    fn visit_gate(&mut self, name: &Span<Symbol>, params: &[Span<Expr>], args: &[Span<Reg>]) -> Result<(), Self::Error> {
        println!("{name:?}: {args:?}");

        let args: Vec<_> = (*args).iter().map(|e| &**e).collect();

        let args: Vec<_> = args.into_iter().map(|e| format!("{}[{}]", e.name, e.index.unwrap())).collect();
